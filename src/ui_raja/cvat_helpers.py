"""Core helpers for working with Raja CVAT annotation exports."""
...
def restructure_blink_dataframe(
    ...
    def finalize_event(ev: dict[str, object]) -> None:
        ...
        if missing_count >= 2:
            warnings.warn(
                f"Skipping blink for type {ev['blink_type']} due to too many missing phases.",
                stacklevel=2,
            )
            return
    ...
    for _, row in df.iterrows():
        blink_type = row["blink_type"]
        phase = row["phase"]
        frame = row[frame_col]

        if current is None:
            current = {"blink_type": blink_type, "start": None, "min": None, "end": None}

        if blink_type != current["blink_type"] or (
    ...
    if current is not None and any(current[key] is not None for key in ("start", "min", "end")):
        finalize_event(current)
...
def unzip_file(zip_path: str, extract_to: str) -> None:
    ...
    else:  # pragma: no cover - defensive helper path
...
def filter_min_labels(df_gt: pd.DataFrame) -> pd.DataFrame:
    """Filter ground truth to keep only labels whose names end with ``_min``."""

    return df_gt[df_gt["LabelName"].str.endswith("_min")].copy()


def match_ground_truth_to_annotations(
    df_gtruth: pd.DataFrame,
    df_model: pd.DataFrame,
) -> pd.DataFrame:
    """Match each ground-truth frame to the first available overlapping annotation."""

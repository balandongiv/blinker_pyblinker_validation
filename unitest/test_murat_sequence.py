...
def test_build_overall_summary_combines_metrics():
    summary = pd.DataFrame(
        {
            "recording_id": ["rec01", "rec02"],
            "py_count": [3.0, 2.0],
            "blinker_count": [2.0, 2.0],
            "matched": [2.0, 1.0],
            "py_only": [1.0, 1.0],
            "blinker_only": [0.0, 1.0],
            "pairs_outside_tolerance": [0.0, 1.0],
            "precision": [2 / 3, 0.5],
            "recall": [1.0, 0.5],
            "f1": [0.8, 0.5],
            "onset_mae_sec": [0.1, 0.2],
        }
    )

    overall = _build_overall_summary(summary)

    assert overall["recording_count"] == 2
    assert math.isclose(overall["py_count_total"], 5.0)
    assert math.isclose(overall["blinker_count_total"], 4.0)
    assert math.isclose(overall["precision_micro"], 3 / (3 + 2))
    assert math.isclose(overall["recall_micro"], 3 / (3 + 1))
    assert math.isclose(overall["onset_mae_sec_mean"], 0.15)
